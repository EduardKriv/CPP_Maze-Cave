...
bool IsEqual(s21::Caves *a, s21::Caves *b) {
...
TEST(caves, generation_1) {
  size_t rows = 10;
  size_t cols = 10;
  s21::Caves c(rows, cols, rand() % 100);
...
TEST(caves, open_file_1) {
  s21::Caves c("./example/example_cave_1.txt");
  ASSERT_EQ(c.GetRows(), 10);
  ASSERT_EQ(c.GetCols(), 10);
}

TEST(caves, open_file_2) {
  s21::Caves c("./example/example_cave_2.txt");
  ASSERT_EQ(c.GetRows(), 20);
  ASSERT_EQ(c.GetCols(), 20);
}

TEST(caves, open_file_3) {
  s21::Caves c("./example/example_cave_3.txt");
  ASSERT_EQ(c.GetRows(), 50);
  ASSERT_EQ(c.GetCols(), 50);
}

TEST(caves, metamorf) {
  s21::Caves test("./example/example_cave_1.txt");
  s21::Caves orig("./example/example_cave_1_result.txt");
